...
export default async function AdminPage({
  ...
  // Hall of Fame tab
  let hofSeasons: { id: string; year: string; champion_name: string | null; champion_captain: string | null; runner_up_name: string | null; cup_holder_name: string | null; mvp_name: string | null; mvp_stats: string | null }[] = [];
  let hofRecords: { id: string; title: string; holder: string | null; value: string | null }[] = [];
  if (tab === 'halloffame') {
    const [{ data: s }, { data: r }] = await Promise.all([
      supabaseAdmin.from('league_history_seasons').select('id,year,champion_name,champion_captain,runner_up_name,cup_holder_name,mvp_name,mvp_stats').order('year', { ascending: false }),
