  ...
  try {
    ...
    const toneInstructions: Record<string, string> = {
      ...
      end_season:
        ...
        `הבחן בבירור בין שלושה תארים נפרדים: (א) מובילות הבתים בעונה הסדירה (צפון/דרום) — הישג של דירוג בטבלה בלבד; ` +
        `(ב) אלופת הליגה — נקבעת אך ורק בפלייאוף שמתחיל לאחר תום העונה הסדירה, ולא לפי המקום בטבלה; ` +
        `(ג) זוכת הגביע — תחרות נפרדת. ` +
        (playoffChampion
          ? `אלופת הליגה העונה היא ${playoffChampion} (מנצחת הפלייאוף) — חגוג אותה כאלופת הליגה. `
          : `הפלייאוף טרם הוכרע — אל תכתיר אף קבוצה כאלופת הליגה, וציין שהאליפות תוכרע בפלייאוף. `) +
        `אל תכנה את מובילת העונה הסדירה או את זוכת הגביע "אלופת הליגה" — ציין כל תואר בנפרד ובמדויק. ` +
        `נתח את הבולטות בעונה, הדגש את מובילי הניקוד, וסיים במבט קדימה.`,
      playoffs:
...
${cupScorerLines || '(אין נתוני ניקוד לגביע)'}`;
    ...
    // End-of-season wrap-up gets the regular season AND the playoff outcome, so
    // the champion is the playoff winner — never the regular-season table leader.
    const endSeasonSections =
`${leagueSections}

== תוצאות הפלייאוף (עונת ${dataSeason}) ==
${playoffSeriesLines || '(אין נתוני פלייאוף)'}${playoffChampion
  ? `\n\nאלופת הליגה (מנצחת הפלייאוף): ${playoffChampion}`
  : '\n\n(אלופת הליגה טרם הוכרעה — הפלייאוף בעיצומו או טרם הוזן)'}`;

    const dataSections =
      reviewType === 'playoffs'   ? playoffSections :
      reviewType === 'cup'        ? cupSections :
      reviewType === 'end_season' ? endSeasonSections :
      leagueSections;
    ...
    const prompt = `אתה עיתונאי ספורט ותיק המתמחה בכדורסל ישראלי. ` +
      ...
      `מבנה התחרות: (1) עונה סדירה — קובעת את דירוג הבתים (צפון/דרום) בלבד; (2) פלייאוף — מתחיל בתום העונה הסדירה וקובע את אלופת הליגה; (3) גביע — תחרות נפרדת עם מחזיקת גביע משלה. ` +
      `"אלופת הליגה" = מנצחת הפלייאוף בלבד. לעולם אל תכנה את מובילת העונה הסדירה או את זוכת הגביע "אלופת הליגה".
