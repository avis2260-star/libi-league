...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const toneInstructions: Record<string, string> = {
      ...
      end_season:
        `זוהי סקירת סיום העונה הסדירה ${targetSeason} — התמקד אך ורק בעונה הסדירה ובמה שקרה בה. ` +
        `סכם את העונה הסדירה: מובילות הבתים (צפון/דרום) והמרדף בצמרת, ההפתעות והאכזבות, מאזנים ופערי סלים בולטים, ומובילי הניקוד. ` +
        `אסור בהחלט: אל תכריז על אלופת ליגה, ואל תזכיר את הפלייאוף או את תוצאותיו/מנצחתו — אלופת הליגה נקבעת בפלייאוף שמתקיים לאחר מכן, וזה אינו נושא הסקירה הזו. ` +
        `אל תתמקד בגביע (יש לו סקירה נפרדת). מותר לסיים במשפט מבט-קדימה קצר לקראת הפלייאוף — מבלי לנקוב בשום זוכה.`,
      playoffs:
...
${cupScorerLines || '(אין נתוני ניקוד לגביע)'}`;
    ...
    // End-of-season wrap-up = the REGULAR SEASON only. No cup, no playoffs — those
    // have their own categories — and it must not crown a league champion.
    const endSeasonSections =
`== טבלת הליגה — סיום העונה הסדירה (עונת ${dataSeason}) ==
${standingLines || '(אין נתוני טבלה)'}

== תוצאות משחקי העונה הסדירה ==
${gameResultLines || '(אין תוצאות)'}

== מובילי ניקוד בעונה הסדירה ==
${scorerLines || '(אין נתוני ניקוד)'}`;
